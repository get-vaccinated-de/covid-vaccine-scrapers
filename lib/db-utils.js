...
async function writeLocationsByRefIds(refIds, locations) {
    const queries = refIds.map((refId, idx) =>
        fq.Create(fq.Ref(fq.Collection("locations"), refId), {
            data: {
                name: locations[idx].name,
                address: {
                    street: locations[idx].street,
                    city: locations[idx].city,
                    zip: locations[idx].zip,
                },
                signUpLink: locations[idx].signUpLink,
